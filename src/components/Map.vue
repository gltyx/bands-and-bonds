...
const points = computed(() => {
  ...
  const steps = ['Go straight', 'Turn right', 'Turn right'];
...
<template>
  <div class="map" @click="onClick" ref="mapElement">
    ...
    <template v-for="point in allPoints">
      <img v-if="point.type !== 'none'" :alt="point.name" :style="style(point)"
        @mouseenter="pos = { x: point.x, y: point.y }" :src="`/images/generated/${icon(point)}-outlined.webp`"
        :class="{ marker: true, undiscovered: !points.includes(point) }" />
    </template>
  </div>
</template>
